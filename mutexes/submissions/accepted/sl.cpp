...
int main() {
	...
	function<int(int, B)> simulate = [&](int i, B cur) -> int {
		trav(ins, bodies[i]) {
			int x = ins.second;
			if (ins.first == ACQUIRE) {
				if (cur[x] == 1)
				...
				B zi = zeroins[x], oi = oneins[x];
				B zo = zeroouts[x], oo = oneouts[x];
				if (error[x] || (zi & cur).any() || (oi & ~cur).any()) {
					return simulate(x, cur);
				}
				else {
					cur |= oo;
					cur &= ~zo;
	...
	if (!error[main]) {
		...
		int e = simulate(main, B());
		if (e == ACQUIRE) cout << "deadlock" << endl;
		else if (e == RELEASE) cout << "error" << endl;
		else if (e == ACCESS) cout << "corruption" << endl;
		else assert(0);
	}
}
